...
export default function EditFormPage() {
  ...
  const fetchForm = async () => {
    try {
      ...
      if (!res.ok) {
        throw new Error(result.error || 'Ralat memuatkan borang');
      }
      ...
      setStartDate(form.start_date ? form.start_date.slice(0, 16) : '');
      setEndDate(form.end_date ? form.end_date.slice(0, 16) : '');
    } catch (err: any) {
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            <span className="badge bg-light text-dark me-2">/{slug}</span>
            <button className="btn btn-link btn-sm p-0" onClick={copyPublicLink}>
          ...
          <div className="row g-3">
            ...
            <div className="col-md-4">
              ...
              <div className="input-group">
                <span className="input-group-text">/forms/</span>
                <input
                  type="text"
                  className="form-control"
                  value={slug}
                  onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-'))}
                  placeholder="pendaftaran-tadarus"
                />
              </div>
            </div>
