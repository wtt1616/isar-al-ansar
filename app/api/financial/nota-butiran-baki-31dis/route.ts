...
// GET - Fetch all nota butiran baki 31 Dis for a specific year
export async function GET(request: NextRequest) {
  try {
    ...
    const [bakiBank] = await pool.query<BakiBankRow[]>(
      'SELECT * FROM nota_baki_bank_31dis WHERE tahun = ? ORDER BY id',
      [tahun]
    );

    // Fetch investment balances
    const [bakiPelaburan] = await pool.query<BakiPelaburanRow[]>(
      'SELECT * FROM nota_baki_pelaburan_31dis WHERE tahun = ? ORDER BY id',
      [tahun]
    );

    // Fetch deposit balances
    const [bakiDeposit] = await pool.query<BakiDepositRow[]>(
      'SELECT * FROM nota_baki_deposit_31dis WHERE tahun = ? ORDER BY id',
      [tahun]
    );
    ...
    console.error('Error fetching nota butiran baki 31 Dis:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (type) {
      case 'bank':
        [result] = await pool.query<ResultSetHeader>(
          `INSERT INTO nota_baki_bank_31dis (tahun, nama_bank, cawangan, baki_tahun_semasa, baki_tahun_sebelum, created_by)
           VALUES (?, ?, ?, ?, ?, ?)`,
      ...
      case 'pelaburan':
        [result] = await pool.query<ResultSetHeader>(
          `INSERT INTO nota_baki_pelaburan_31dis (tahun, nama_bank, cawangan, baki_tahun_semasa, baki_tahun_sebelum, created_by)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [tahun, data.nama_bank, data.cawangan || null, data.baki_tahun_semasa || 0, data.baki_tahun_sebelum || 0, session.user.id]
        );
        break;
      ...
      case 'deposit':
        [result] = await pool.query<ResultSetHeader>(
          `INSERT INTO nota_baki_deposit_31dis (tahun, deposit, tarikh, baki_tahun_semasa, baki_tahun_sebelum, created_by)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [tahun, data.deposit, data.tarikh || null, data.baki_tahun_semasa || 0, data.baki_tahun_sebelum || 0, session.user.id]
        );
        break;
    ...
    console.error('Error creating nota butiran baki 31 Dis:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    switch (type) {
      ...
      case 'pelaburan':
        await pool.query(
          `UPDATE nota_baki_pelaburan_31dis
           SET nama_bank = ?, cawangan = ?, baki_tahun_semasa = ?, baki_tahun_sebelum = ?, updated_by = ?
           WHERE id = ?`,
          [data.nama_bank, data.cawangan || null, data.baki_tahun_semasa || 0, data.baki_tahun_sebelum || 0, session.user.id, id]
        );
      ...
      case 'deposit':
        await pool.query(
          `UPDATE nota_baki_deposit_31dis
           SET deposit = ?, tarikh = ?, baki_tahun_semasa = ?, baki_tahun_sebelum = ?, updated_by = ?
           WHERE id = ?`,
          [data.deposit, data.tarikh || null, data.baki_tahun_semasa || 0, data.baki_tahun_sebelum || 0, session.user.id, id]
        );
        break;
    ...
    console.error('Error updating nota butiran baki 31 Dis:', error);
  ...
  try {
    ...
    switch (type) {
      ...
      case 'pelaburan':
        ...
        break;
      case 'deposit':
        tableName = 'nota_baki_deposit_31dis';
        break;
      default:
        return NextResponse.json({ error: 'Invalid type' }, { status: 400 });
    }
    ...
    console.error('Error deleting nota butiran baki 31 Dis:', error);
