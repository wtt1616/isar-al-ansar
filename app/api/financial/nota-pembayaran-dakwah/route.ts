...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'auto_generate') {
      ...
      const getYearlyPembayaranDakwah = async (year: number) => {
      ...
      const currentYearSums = await getYearlyPembayaranDakwah(targetYear);
      const previousYearSums = await getYearlyPembayaranDakwah(previousYear);
