...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'auto_generate') {
      ...
      const getYearlyPembayaranSumberManusia = async (year: number) => {
        ...
        for (let month = 1; month <= 12; month++) {
          ...
          const [rows] = await pool.query<TransactionSumRow[]>(
            `SELECT
               COALESCE(sub_category1_pembayaran, 'Emolumen') as sub_category1,
             ...
             GROUP BY COALESCE(sub_category1_pembayaran, 'Emolumen'),
      ...
      const currentYearSums = await getYearlyPembayaranSumberManusia(targetYear);
      const previousYearSums = await getYearlyPembayaranSumberManusia(previousYear);
