...
interface SettingsData {
  sukarelawan_muslimat_tahun_aktif: number;
  sukarelawan_muslimat_pendaftaran_aktif: boolean;
  sukarelawan_muslimat_hari_options: string;
}
...
export default function SukarelawanRamadhanMuslimatAdminPage() {
  ...
  const [settings, setSettings] = useState<SettingsData>({
    sukarelawan_muslimat_tahun_aktif: currentYear,
    sukarelawan_muslimat_pendaftaran_aktif: true,
    sukarelawan_muslimat_hari_options: HARI_OPTIONS.join(',')
  });
  ...
  const fetchSettings = async () => {
    try {
      ...
      setSettings({
        sukarelawan_muslimat_tahun_aktif: data.sukarelawan_muslimat_tahun_aktif || currentYear,
        sukarelawan_muslimat_pendaftaran_aktif: data.sukarelawan_muslimat_pendaftaran_aktif !== false,
        sukarelawan_muslimat_hari_options: data.sukarelawan_muslimat_hari_options || HARI_OPTIONS.join(',')
      });
    } catch (error) {
  ...
  const saveSettings = async () => {
    try {
      ...
      const res = await fetch('/api/settings/sukarelawan-muslimat', {
        ...
        credentials: 'include',
        body: JSON.stringify({
          tahun_aktif: settings.sukarelawan_muslimat_tahun_aktif,
          pendaftaran_aktif: settings.sukarelawan_muslimat_pendaftaran_aktif,
          hari_options: settings.sukarelawan_muslimat_hari_options
        })
      });
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`/api/sukarelawan-ramadhan-muslimat?${params}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/sukarelawan-ramadhan-muslimat?id=${id}`, {
        method: 'DELETE'
      });
    ...
    return (
      <div className="container py-4">
        <div className="d-flex justify-content-center">
          <div className="spinner-border" role="status" style={{ color: '#9c4d8b' }}>
  ...
  return (
    <div className="container-fluid py-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h4 className="mb-1">
            <i className="bi bi-people-fill me-2" style={{ color: '#9c4d8b' }}></i>
            Sukarelawan Ramadhan Muslimat {tahun}
          </h4>
          <p className="text-muted mb-0">Pengurusan pendaftaran sukarelawan muslimat</p>
        </div>
      ...
      {stats && (
        <div className="row g-3 mb-4">
          <div className="col-6 col-md-3">
            <div className="card text-white h-100" style={{ backgroundColor: '#9c4d8b' }}>
      ...
      <div className="alert alert-light border mb-4 d-flex align-items-center justify-content-between">
        <div>
          ...
          <strong>Borang Awam Muslimat:</strong>{' '}
          Tahun <strong>{settings.sukarelawan_muslimat_tahun_aktif}</strong> |{' '}
          {settings.sukarelawan_muslimat_pendaftaran_aktif ? (
      ...
      <div className="row g-3 mb-4">
        ...
        <div className="col-md-6">
          <div className="card h-100">
            <div className="card-header" style={{ backgroundColor: '#f8f0f6' }}>
              <i className="bi bi-calendar-week me-1" style={{ color: '#9c4d8b' }}></i> Statistik Hari Bertugas (Diluluskan)
            </div>
            <div className="card-body">
        ...
        <div className="col-md-6">
          <div className="card h-100">
            <div className="card-header" style={{ backgroundColor: '#f8f0f6' }}>
              <i className="bi bi-tag me-1" style={{ color: '#9c4d8b' }}></i> Statistik Size Baju (Diluluskan)
            </div>
            <div className="card-body">
              <div className="row g-2">
                {sizeStats.map(stat => (
                ...
                {data.length === 0 ? (
                  ...
                  data.map((item, index) => (
                    <tr key={item.id}>
                      ...
                      <td><span className="badge" style={{ backgroundColor: '#9c4d8b' }}>{item.size_baju}</span></td>
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  ...
                  Tetapan Pendaftaran Sukarelawan Muslimat
                </h5>
              ...
              <div className="modal-body">
                <div className="mb-4">
                  ...
                  <input
                    type="number"
                    className="form-control"
                    value={settings.sukarelawan_muslimat_tahun_aktif}
                    onChange={(e) => setSettings(prev => ({
                      ...prev,
                      sukarelawan_muslimat_tahun_aktif: parseInt(e.target.value, 10) || currentYear
                    }))}
                    min="2020"
                    max="2100"
                  ...
                  <div className="form-text">
                    Tahun yang akan dipaparkan pada borang pendaftaran awam muslimat.
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="form-check form-switch">
                    <input
                      ...
                      checked={settings.sukarelawan_muslimat_pendaftaran_aktif}
                      onChange={(e) => setSettings(prev => ({
                        ...prev,
                        sukarelawan_muslimat_pendaftaran_aktif: e.target.checked
                      }))}
                    />
                  ...
                  <div className="form-text">
                    Jika ditutup, borang pendaftaran awam muslimat akan memaparkan mesej pendaftaran ditutup.
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="form-text mb-2">
                    Pilih hari-hari yang akan dipaparkan pada borang pendaftaran awam muslimat.
                  </div>
                  <div className="row g-2">
                    {HARI_OPTIONS.map(hari => {
                      const enabledDays = settings.sukarelawan_muslimat_hari_options.split(',').map(d => d.trim());
                        ...
                        <div key={hari} className="col-6">
                          <div className="form-check">
                            <input
                              ...
                              checked={isEnabled}
                              onChange={(e) => {
                                const currentDays = settings.sukarelawan_muslimat_hari_options.split(',').map(d => d.trim()).filter(d => d);
                                let newDays: string[];
                                if (e.target.checked) {
                                ...
                                setSettings(prev => ({
                                  ...prev,
                                  sukarelawan_muslimat_hari_options: newDays.join(',')
                                }));
                              }}
                            />
                ...
                <div className="alert mb-0" style={{ backgroundColor: '#f8f0f6', borderColor: '#9c4d8b' }}>
                  <i className="bi bi-info-circle me-2" style={{ color: '#9c4d8b' }}></i>
                  <strong>Link borang awam muslimat:</strong>
                  <br />
                  <code className="user-select-all">{typeof window !== 'undefined' ? window.location.origin : ''}/sukarelawan-ramadhan-muslimat</code>
                </div>
              </div>
              <div className="modal-footer">
                <button
